...
def gen_bass(style: str, chords: list[Chord], section_start: float, bars: int,
    ...
    for bar in range(bars):
        ...
        if style == "sustained_roots":
            notes.append(Note(root, local, beats_per_bar, vel - 12))
        elif style == "four_floor_roots":
            for b in range(int(beats_per_bar)):
                notes.append(Note(root, local + b, 0.9, vel))
        elif style == "eighth_pump":
            for i in range(int(beats_per_bar * 2)):
                notes.append(Note(root, local + i * 0.5, 0.45, vel if i % 2 == 0 else vel - 18))
        elif style == "sixteenth_pump":
            for i in range(int(beats_per_bar * 4)):
                v = vel if i % 4 == 0 else (vel - 14 if i % 2 == 0 else vel - 26)
                notes.append(Note(root, local + i * STEP, STEP * 0.85, v))
        elif style == "offbeat_eighths":
            for i in range(int(beats_per_bar * 2)):
                if i % 2 == 1:  # off-beats only — classic EDM pump against the kick
                    notes.append(Note(root, local + i * 0.5, 0.42, vel))
        elif style == "root_fifth_octave":
            seq = [root, root + 12, fifth, root + 12]
            for i in range(int(beats_per_bar * 2)):
                if i % 2 == 1:
                    notes.append(Note(seq[(i // 2) % 4], local + i * 0.5, 0.45, vel - 8))
                elif i == 0:
                    notes.append(Note(root, local, 0.45, vel))
        elif style == "trap_808":
            notes.append(Note(root - 12, local, beats_per_bar * 0.6, vel))
            if bar % 2 == 1:
            ...
            notes.append(Note(fifth - 12 if fifth - 12 > 24 else fifth, local + beats_per_bar / 2,
                              beats_per_bar / 2 - 0.5, vel - 26))
        elif style == "rolling_octaves":
            for i in range(int(beats_per_bar * 2)):
                pitch = root if i % 4 in (0, 1) else root + 12
                notes.append(Note(pitch, local + i * 0.5, 0.48, vel - (0 if i % 2 == 0 else 16)))
        else:
            notes.append(Note(root, local, beats_per_bar, vel - 12))
    return notes
...
def gen_pad(chords: list[Chord], section_start: float, bars: int,
    ...
    while bar < bars:
        local = bar * beats_per_bar
        chord = chord_at(chords, section_start + local)
        # hold each chord for as many whole bars as it stays the same (max 2)
        span = 1
        if bar + 1 < bars:
            nxt = chord_at(chords, section_start + (bar + 1) * beats_per_bar)
            if nxt.root == chord.root and nxt.quality == chord.quality:
                span = 2
        duration = span * beats_per_bar - 0.1
        for p in chord.pitches(octave=4):
            notes.append(Note(p, local, duration, vel))
        # add the 9th for color on sustained pads
        notes.append(Note(chord.pitches(octave=4)[0] + 14, local, duration, vel - 18))
        bar += span
    return notes
